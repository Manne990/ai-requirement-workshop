...
| Auth boundary              | #39      | Browser receives privileged auth or service-role secrets.                                                       | Architecture review confirms only browser-safe public config reaches Vite code; server-only secrets stay in Vercel/Supabase.                               | Fail                             | Architecture exists in `docs/production-architecture.md`; implementation and review still pending.                                    |
| Organization isolation     | #39      | Users can read or modify workshops outside their organization.                                                  | Supabase schema, RLS policies, and policy tests prove tenant isolation.                                                                                    | Fail                             | No production Supabase schema or RLS tests exist yet.                                                                                 |
...
| Durable persistence        | #39      | Production data is browser-only or cannot recover after device loss.                                            | Server-backed workshop rows, attachment metadata, export/import, and backup/recovery procedure are verified.                                               | Fail                             | V4 has local browser persistence plus export/import and local disk mirror only.                                                       |
| AI safety and audit        | #39      | AI calls expose secrets, send excess context, or lack audit evidence.                                           | BFF tests prove tokens are never returned, prompt payload is minimized, JSON is validated, and audit events are written.                                   | Fail                             | Vite local API exists; production BFF and audit writes are pending.                                                                   |
